...
void mmu_read_from_virtual_address(mmu *this, addr32 virtual_address,
    ...
    page_directory* dir = this->page_directories[pid];
    page_directory_entry *entry = &dir->entries[virtual_address >> (VIRTUAL_ADDR_SPACE - NUM_OFFSET_BITS)];

    if (!pte) {
        mmu_tlb_miss(this);
        if (!entry->present) {
            mmu_raise_page_fault(this);
            entry->base_addr = ask_kernel_for_frame(NULL) >> NUM_OFFSET_BITS;
            read_page_from_disk((page_table_entry*)entry);
            entry->present = 1;
            entry->read_write = 1;
            entry->user_supervisor = 0;
        }
        page_table *table = (page_table*) get_system_pointer_from_pde(entry);
        pte = &(table->entries[(virtual_address & 0x003FF000) >> NUM_OFFSET_BITS]);
    }
...
void mmu_write_to_virtual_address(mmu *this, addr32 virtual_address, size_t pid,
    ...
    if (!pte) {
        ...
        page_table *pt = (page_table *)get_system_pointer_from_pde(entry);
